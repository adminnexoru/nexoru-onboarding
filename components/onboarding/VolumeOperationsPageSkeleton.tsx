    ...
    <div className="rounded-[28px] border border-[#E5E7EB] bg-white px-5 py-7 shadow-sm sm:rounded-[32px] sm:px-8 sm:py-9 md:px-12 md:py-12">
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-8 md:grid-cols-2">
        {Array.from({ length: 6 }).map((_, index) => (
          <div key={index}>
            <div className="mb-3 h-5 w-48 animate-pulse rounded bg-[#E5E7EB]" />
            <div className="mb-3 h-5 w-full max-w-[320px] animate-pulse rounded bg-[#E5E7EB]" />
            <div className={`${index === 5 ? "h-[140px]" : "h-14"} w-full animate-pulse rounded-2xl bg-[#E5E7EB]`} />
          </div>
        ))}
      </div>
