...
**Decision**: Implement the "wait a short pause, then reply once" behavior (spec FR-018) **inside the request lifecycle of the webhook function itself**, coordinated through a row in Postgres — no external queue/scheduler service.

Mechanism: on each inbound message, the handler upserts a `WhatsAppConversation` buffer with an incremented `bufferVersion` and the message appended, then sleeps ~2–3 seconds (well within Vercel Functions' current default execution timeout), then re-reads the row. If `bufferVersion` has advanced past what it read after waking up, another (newer) inbound message arrived during the wait — that means a *later* invocation is now the one responsible for replying, so this invocation exits without sending anything. Only the invocation whose wait wasn't "beaten" by a newer message goes on to compose and send the single combined reply, then clears the buffer.

**Rationale**: This needs zero new infrastructure — no queue, no cron, no third-party scheduler — which matches the "simplest solution" clause in Calidad y Flujo de Entrega and avoids a new paid dependency under Minimal Operational Cost. Vercel's Node.js runtime (Fluid Compute) supports holding a function open for the few seconds this requires. The correctness-critical part is the atomic version check (a single `UPDATE ... RETURNING` or equivalent, not read-then-write) so two concurrent invocations can't both decide they're "the latest."

**Alternatives considered**: Upstash QStash (or a similar delayed-delivery queue) scheduling a callback at T+3s — this is a clean, well-tested pattern and was seriously considered, but rejected for Phase 1 because it adds a new external paid service (Upstash's free tier would likely cover this low volume, but it's still a new moving part) for something achievable with the database already in use. Revisit if Phase 1 testing shows the in-process wait approach causing function-cost or reliability issues at higher volume.
