...
- The escalation email to `admin@nexoru.ai` (already sent per FR-012) includes a link to `/whatsapp/escalations/[conversationId]/reply`, carrying a signed token (HMAC of the conversation id + a 48-hour expiry, using a secret env var — the same pattern this repo already uses for `INTERNAL_API_KEY` on `/api/update-meeting`) instead of building a login system, since this app has no user-auth model (`CLAUDE.md`: "There is no authentication"). 48 hours was chosen to comfortably outlast WhatsApp's 24-hour customer-service window (which separately gates whether a free-form reply through this page can even be sent — see tasks.md's Phase 5 notes) without being open-ended.
...
- **Claude's role is scoped to two narrow, schema-constrained calls**: (1) classify the inbound message's intent into a fixed enum — `ask_about_proposal | request_human | unclear` (contracts/whatsapp-webhook.md; deliberately excludes reference confirmation and unsupported-format handling, since both of those are resolved by deterministic code — `extractReference`/`phoneMatches` and the message-type check — before `classifyIntent` is ever invoked) — and (2) compose the natural-language reply text *strictly from fields the caller already fetched from the database* (appointment date/time/reference/link, or proposal package/price/scope) — never asked to invent, look up, or decide on data it wasn't handed. Grounding in FR-009 is enforced this way: the composition prompt receives only the literal DB fields to phrase, so there's nothing to "answer generically" from.
...
- **Separate per-number limit on the escalation *alert* itself: at most 1 WhatsApp+email alert to Ulises per hour, per prospect phone number.** This is distinct from the message-processing rate limit above — it caps notification noise, not conversation handling. Before sending the alerts in FR-012, check `WhatsAppEscalation` for that phone number's most recent `whatsappAlertSentAt`/`emailSentAt` within the last hour; if one exists, still create the `WhatsAppEscalation` row (audit trail, FR-017) and still move the conversation to `ESCALATED` (the safety behavior — agent goes silent — is never suppressed, only the repeat notification is), but skip the actual send and mark the row `rateLimited: true` instead. This prevents a chatty or misbehaving conversation (e.g., a rapid sequence of stuck points right after a human hand-back) from paging Ulises repeatedly within the same hour, while the very first escalation in any given hour always still reaches him.

**Rationale**: Ties directly to spec FR-009's grounding requirement and to Principle V. Keeping the model out of the extraction/decision path for anything security- or data-access-relevant means a successful prompt injection can, at worst, produce an oddly-worded reply — it cannot make the agent confirm someone else's appointment or leak someone else's proposal, because those decisions never touch the model. The escalation-alert limit protects Ulises's own attention as a scarce resource, separately from protecting the prospect from being ignored.
...
## 9. Prompt caching eligibility and per-call cost logging

**Decision**: Do not enable prompt caching on `composeReply`/`classifyIntent` for this feature, and record this as a deliberate, sourced deferral rather than an oversight. Record token usage and an estimated cost for every one of these two calls, in a shape that can be summed by calendar month.

**Prompt caching — checked against Anthropic's current published documentation, not assumed**: minimum cacheable prompt length is model-specific and enforced silently (a prompt below the minimum is simply processed without caching, no error) — for the Haiku family specifically, **Haiku 4.5 requires 4,096 tokens minimum** to cache anything at all (Haiku 3.5, now retired, required 2,048). This feature's two Claude calls are short by design (decision 7's own cost estimate: a few hundred to ~1.5k input tokens, ~100–300 output tokens) — every one of them stays under the 4,096-token floor, so enabling `cache_control` on them would have zero effect (no `cache_creation_input_tokens`/`cache_read_input_tokens` would ever appear in the response) while adding code complexity for nothing. This satisfies the constitution's "MUST usar prompt caching cuando el proveedor lo soporte" as a *checked and documented* conclusion — the provider supports caching in general, but not for prompts this short on this model, which is what "lo soporte" has to mean in practice for it to be a meaningful test rather than a blanket mandate to add `cache_control` regardless of effect. Revisit if a future change (e.g., a longer system prompt, or switching `composeReply` to a larger model per research.md's own noted fallback) pushes either call's prompt size above the threshold.

**Cost logging, summable monthly (constitution Principle IV, spec FR-025)**: every `classifyIntent`/`composeReply` call writes one row to a new `WhatsAppAiCall` table (data-model.md) — `kind`, `model`, `inputTokens`, `outputTokens`, `estimatedCostUsd` (computed from the call's actual token counts against Haiku's published per-token rate at call time, not a flat estimate), `createdAt`. Comparing this feature's real spend against the $600/month baseline (SC-003) is then `SELECT date_trunc('month', "createdAt"), SUM("estimatedCostUsd") FROM "WhatsAppAiCall" GROUP BY 1` — a plain aggregate query, no new reporting infrastructure. This table is deliberately **not** subject to the 90-day conversation-retention rule (decision/spec FR-022): it holds no phone numbers, no message text, and no other prospect personal data — only operational cost figures needed for an ongoing business comparison that outlives any single conversation's 90-day window.

**Alternatives considered**: Enabling `cache_control` anyway "just in case" — rejected as pointless overhead below the documented minimum, and contrary to the constitution's own "no se agregan capas... sin necesidad demostrada" clause. A flat per-call cost estimate instead of computing from actual returned token counts — rejected as less accurate for a metric whose entire purpose is an honest comparison against a real dollar baseline.
